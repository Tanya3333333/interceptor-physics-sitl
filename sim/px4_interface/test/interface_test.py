...
from analysis.loggerModu import SimLogger, ServerLogger
...
class TestPX4WaypointsMission(unittest.TestCase):  
    ...
    def test_fdm_init_and_step(self):
        ...
        last_fdm_step = perf_counter_ns()
        last_sensor_send = perf_counter_ns()
        last_state_quat_send = perf_counter_ns()
        last_gps_send = perf_counter_ns()
        last_heartbeat_send = perf_counter_ns()
        
        
        t_end_sim = (perf_counter_ns()/1e9) + 1000  # seconds
        while (perf_counter_ns()/1e9) < t_end_sim:
